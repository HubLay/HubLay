...
void Class_Tricycle_Chassis::Init(float __Velocity_X_Max, float __Velocity_Y_Max, float __Omega_Max, float __Steer_Power_Ratio)
{
    ...
    {
        #ifdef Test_Gimbal
            Motor_Wheel[i].IMU = &Boardc_BMI;
        #endif

        Motor_Wheel[i].PID_Angle.Init(5.0f, 0.0f, 0.0f, 0.0f,16000,16000);
        Motor_Wheel[i].PID_Omega.Init(5.0f, 0.0f, 0.0f, 0.0f, Motor_Wheel[i].Get_Output_Max(), Motor_Wheel[i].Get_Output_Max());
    }
...
void Class_Tricycle_Chassis::Speed_Resolution(){
        ...
        //配置为角速度控制模式
        for (int i = 0; i < 4; i++){
            Motor_Wheel[i].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_ANGLE);
        }
        //设定目标 直接给到电机输出轴,闭环下
        
        Motor_Wheel[0].Set_Target_Omega_Radian(  M1_Omega);
        Motor_Wheel[1].Set_Target_Omega_Radian(- M2_Omega);
        Motor_Wheel[2].Set_Target_Omega_Radian(- M3_Omega);
        Motor_Wheel[3].Set_Target_Omega_Radian(  M4_Omega);

        Motor_Wheel[0].Set_Target_Radian(M1_Rad);
        Motor_Wheel[1].Set_Target_Radian(M2_Rad);
        Motor_Wheel[2].Set_Target_Radian(M3_Rad);
        Motor_Wheel[3].Set_Target_Radian(M4_Rad);
