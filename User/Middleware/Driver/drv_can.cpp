...
void can_filter_mask_config(CAN_HandleTypeDef *hcan,uint8_t Object_Para, uint32_t ID, uint32_t Mask_ID)
{
    ...
    //从机模式选择开始单元 , 前14个在CAN1, 后14个在CAN2，，前14个滤波器分给can1，后14个给can2
 ...
 *
 * @param hcan CAN编号
 * @param Callback_Function 处理回调函数，每个can通道对应的数据处理函数，将接收到的数据放入对应数组
 */
void CAN_Init(CAN_HandleTypeDef *hcan, CAN_Call_Back Callback_Function)
{
    if (hcan->Instance == CAN1)
    {
		...
		//can_filter_mask_config(hcan, CAN_FILTER(0) | CAN_FIFO_0 | CAN_STDID | CAN_DATA_TYPE, 0 ,0);     //自己更改的配置部分，掩码和ID
...
/**
 * @brief CAN的TIM定时器中断发送回调函数,将消息在TIM5回调函数中统一打包发送
 *
 */
