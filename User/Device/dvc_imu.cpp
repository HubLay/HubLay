...
void Class_IMU::Init()
{
    ...
    // 初始化MahonyAHRS算法，并传入初始四元数，四元数数组初始化为1，0，0，0，即旋转角度为0，不旋转
...
void Class_IMU::TIM_Calculate_PeriodElapsedCallback(void)
{
    ...
    BodyFrameToEarthFrame(INS.MotionAccel_n, INS.MotionAccel_b, INS.q); // 转换回导航系n
...
//IMU失能下，PID运算里会把角度Target控制在当前角度
...
/**
 * @brief          Transform 3dvector from BodyFrame to EarthFrame（机体坐标系-->大地固连坐标系）
...
/**
 * @brief          Transform 3dvector from EarthFrame to BodyFrame （大地固连坐标系-->机体坐标系）
