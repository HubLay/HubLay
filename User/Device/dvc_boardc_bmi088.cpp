...
uint8_t Class_BoardC_BMI::init(SPI_HandleTypeDef *hspi ,IMU_Data_t *__BMI088)
{
	...
	Delay_Init();
    while (BMI088_Init(__BMI088));          //开始真正的初始化与校准
	return 1;
}
...
uint8_t Class_BoardC_BMI::BMI088_Init(IMU_Data_t *__BMI088)
{
    ...
    if (caliOffset)
        Calibrate_MPU_Offset(__BMI088);         //开始校准
    else
    {
    ...
    do
    {
        ...
        for (uint16_t i = 0; i < CaliTimes; i++)
        {
            ...
            if (gNormDiff > 0.5f ||
                ...
                gyroDiff[2] > 0.15f)
                break;
            DWT_Delay(0.0005);                  //最大读取频率2000Hz
        }

        // 取平均值得到标定结果         acc校准一个重力加速度，gyro分别校准三轴
        ...
        caliCount++;
             ...
             fabsf(bmi088->GyroOffset[2]) > 0.01f);             //只要没超时，校准错误，重新校准

    // 根据标定结果校准加速度计标度因数，校准因数
...
uint8_t Class_BoardC_BMI::BMI088_Accel_Init(void)
{
    ...
    //check commiunication  --->    用ID反馈检测通信是否正常
