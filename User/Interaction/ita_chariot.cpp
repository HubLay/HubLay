...
{
    #if defined(Test_Chassis) || defined(Test_Gimbal)
        Chassis.Init();
...
/**
 * @brief can回调函数处理云台发来的数据，控制电机运动的Target
 *
 */
#ifdef CHASSIS    
    //控制类型字节
...
{
    #ifdef Test_Gimbal
        Chassis.Motor_Wheel[1].Set_Target_Angle(0.0f);
        for (int i = 0; i < 4; i++){
            Chassis.Motor_Wheel[i].Set_DJI_Motor_Control_Method(DJI_Motor_Control_Method_IMU_ANGLE);
            Chassis.Motor_Wheel[i].Transform_Angle();
        }

        for (int i = 0; i < 4; i++){
            Chassis.Motor_Wheel[i].TIM_PID_PeriodElapsedCallback();
        }
...
void Class_Chariot::TIM1msMod50_Alive_PeriodElapsedCallback()
{
    ...
    if (mod50 == 50)
    {
        mod50_mod3++;
            ...
            //循环四个轮子的电机，判断四个电机数据发送是否存活，C++语法特性，范围运算符
            ...
            Chassis.Boardc_BMI.TIM1msMod50_Alive_PeriodElapsedCallback();
